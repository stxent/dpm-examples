...
/*----------------------------------------------------------------------------*/
struct Interface *boardSetupWS281x(size_t size)
{
  const struct WS281xSspConfig config = {
      .size = size,
      .mosi = PIN(0, 9),
      .priority = 0,
      .channel = 0
  ...
  struct Interface * const interface = init(WS281xSsp, &config);
  assert(interface != NULL);
  return interface;
}
