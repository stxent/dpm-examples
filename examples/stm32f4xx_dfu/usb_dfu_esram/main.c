/*
 * stm32f4xx_dfu/usb_dfu_esram/main.c
...
/*----------------------------------------------------------------------------*/
static const char productStringEn[] = "STM32F4xx DFU for ESRAM";
struct Board instance;
/*----------------------------------------------------------------------------*/
static void boardInit(struct Board *board)
{
  boardSetupClockPll();
  boardSetupMemoryESRAM(&board->memoryPackage);
...
static void boardDeinit(struct Board *board)
{
  ...
  /* FSMC driver should be left in an initialized state */
}
/*----------------------------------------------------------------------------*/
static void customStringHeader(const void *, enum UsbLangId,
...
static void startFirmware(struct Board *board)
{
  const uint32_t *table =
      fsmcSramAddress((const struct FsmcSram *)board->memoryPackage.lower);
